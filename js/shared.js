...
// ==================== LOGIN ====================
let cdSelecionadoLogin = '';

function selecionarCdLogin(cd) {
  cdSelecionadoLogin = cd;
  document.querySelectorAll('.login-cd-card').forEach(el => {
    el.classList.toggle('selected', el.dataset.cd === cd);
  });
}

function loginFieldBlur(el) {
  el.closest('.login-field').classList.remove('focus');
  if (el.value) {
    el.closest('.login-field').classList.add('has-value');
  } else {
    el.closest('.login-field').classList.remove('has-value');
  }
}
...
function popularSelectLogin() {
  ...
  const ativos = todosUsuarios.filter(u => u.ativo);
  ativos.forEach(u => {
    const opt = document.createElement('option');
    opt.value = u.nome;
  ...
  if (current && ativos.some(u => u.nome === current)) sel.value = current;
}
...
function telaLogin() {
  ...
  cdSelecionadoLogin = '';
  document.querySelectorAll('.login-cd-card').forEach(el => el.classList.remove('selected'));
  const selUsuario = document.getElementById('loginUsuario');
  if (selUsuario) {
    selUsuario.value = '';
    selUsuario.closest('.login-field').classList.remove('has-value');
  }
  setTimeout(() => {
    if (selUsuario) selUsuario.focus();
  }, 100);
}
...
async function validarLogin() {
  ...
  if (!nome) {
    erroEl.textContent = 'Informe o usuário!';
  ...
  if (!cdSelecionadoLogin) {
    erroEl.textContent = 'Selecione um CD!';
    erroEl.classList.add('show');
    return;
  }

  const user = todosUsuarios.find(u => u.nome === nome && u.ativo);
  if (!user) {
  ...
  sessaoLogin(nome, cdSelecionadoLogin);
